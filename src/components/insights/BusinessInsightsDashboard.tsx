...
const TrendIndicator = memo(
  ({
    ...
    if (trend === "rising")
      return <ArrowUpRight className={`${iconSize} text-green-400`} />;
    if (trend === "declining")
    ...
    return (
      <div className="flex items-center gap-2 p-2 bg-slate-800/30 rounded-lg hover:bg-slate-800/50 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-xs text-gray-400">ID: {recipeId}</span>
        </div>
        {getTrendIcon()}
        <Badge
          variant="outline"
        ...
        <div className="hidden sm:flex items-center gap-1">
          <Badge
            variant="outline"
            className="bg-pink-500/10 border-pink-500/30 text-pink-300 text-xs p-1"
          >
            <Heart className="h-3 w-3" /> {favouriteCount}
          </Badge>
          <Badge
            variant="outline"
            className="bg-purple-500/10 border-purple-500/30 text-purple-300 text-xs p-1 hidden md:flex"
          >
            <FolderOpen className="h-3 w-3" /> {collectionCount}
          </Badge>
          <Badge
            variant="outline"
            className="bg-blue-500/10 border-blue-500/30 text-blue-300 text-xs p-1 hidden lg:flex"
          >
    ...
    >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <StatCard
            ...
            color="cyan"
            trend={{
              value: stats.predictions?.projectedGrowthRate || 0,
              label: "projected",
              isPositive: (stats.predictions?.projectedGrowthRate || 0) >= 0,
            }}
          />
          <StatCard
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-bold text-white capitalize">
                  {stats.trends?.userGrowthTrend || "stable"}
                </p>
              </div>
              <TrendIndicator
                trend={stats.trends?.userGrowthTrend || "stable"}
                size="md"
              />
            </CardContent>
          </Card>
          <Card className="bg-gradient-to-br from-emerald-900/30 to-green-900/30 border-emerald-500/30 backdrop-blur-md">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-bold text-white capitalize">
                  {stats.trends?.engagementTrend || "stable"}
                </p>
              </div>
              <TrendIndicator
                trend={stats.trends?.engagementTrend || "stable"}
                size="md"
              />
            </CardContent>
          </Card>
          <StatCard
            ...
            value={`${stats.trends?.favouritesTrend >= 0 ? "+" : ""}${
              stats.trends?.favouritesTrend || 0
            }%`}
          ...
          <StatCard
            ...
            value={`${stats.trends?.collectionsTrend >= 0 ? "+" : ""}${
              stats.trends?.collectionsTrend || 0
            }%`}
            subtitle="vs last week"
          ...
          <StatCard
            ...
            value={`${stats.trends?.mealPlansTrend >= 0 ? "+" : ""}${
              stats.trends?.mealPlansTrend || 0
            }%`}
            subtitle="vs last week"
      ...
      <div>
        <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          ...
          <Badge
            variant="outline"
            className="bg-cyan-500/10 border-cyan-500/30 text-cyan-300 text-xs ml-2"
          >
      ...
      <div>
        <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          ...
          <Badge
            variant="outline"
            className="bg-pink-500/10 border-pink-500/30 text-pink-300 text-xs ml-2"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            badges={[
              {
                label: "Unique",
                value: stats.engagement?.totalUniqueRecipesSaved || 0,
          ...
          <StatCard
            ...
            badges={[
              {
                label: "Avg",
                value: stats.recipes?.avgItemsPerCollection || 0,
          ...
          <StatCard
            ...
            badges={[
              {
                label: "Avg",
                value: stats.recipes?.avgMealsPerPlan || 0,
          ...
          <StatCard
            ...
            badges={[
              {
                label: "Done",
                value: stats.recipes?.completedShoppingLists || 0,
                color:
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Recipe Notes"
          value={stats.recipes?.totalNotes || 0}
          icon={<FileText className="h-6 w-6" />}
          color="amber"
        />
        <StatCard
          title="Filter Presets"
          value={stats.recipes?.totalFilterPresets || 0}
          icon={<Sparkles className="h-6 w-6" />}
          color="indigo"
        />
        <StatCard
          title="Recipe Images"
          value={stats.content?.totalRecipeImages || 0}
          icon={<ImageIcon className="h-6 w-6" />}
          color="cyan"
        />
        <StatCard
          title="Recipe Videos"
          value={stats.content?.totalRecipeVideos || 0}
          ...
          <CardContent className="space-y-1 max-h-80 overflow-y-auto">
            {stats.topContributors?.length > 0 ? (
              stats.topContributors.map((c, i) => (
                <TopContributorItem key={c.email} rank={i + 1} {...c} />
              ))
            ) : (
              <p className="text-center text-gray-400 py-4">
                No contributors yet
      ...
      <Card className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border-slate-500/30 backdrop-blur-md">
        ...
        <CardContent>
          <div className="flex flex-wrap items-center gap-2">
            <Badge
              className={`${
                stats.systemHealth?.databaseStatus === "healthy"
                  ? "bg-green-500/20 text-green-300 border-green-500/30"
                  : "bg-red-500/20 text-red-300 border-red-500/30"
              }`}
            >
              <Database className="h-3 w-3 mr-1" />
              DB: {stats.systemHealth?.databaseStatus || "unknown"}
            </Badge>
            ...
            <Badge className="bg-purple-500/20 text-purple-300 border-purple-500/30">
              ...
              Records:{" "}
              {stats.systemHealth?.totalDbRecords?.toLocaleString() || "N/A"}
            </Badge>
            <Badge className="bg-cyan-500/20 text-cyan-300 border-cyan-500/30">
              <Server className="h-3 w-3 mr-1" />
              Response: {stats.systemHealth?.responseTime || 0}ms
            </Badge>
            <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-500/30">
              <CheckCircle2 className="h-3 w-3 mr-1" />
              Error Rate: {stats.systemHealth?.errorRate || 0}%
            </Badge>
