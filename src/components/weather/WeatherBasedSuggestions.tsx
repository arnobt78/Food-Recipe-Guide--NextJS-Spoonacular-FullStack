...
import { Cloud, MapPin, RefreshCw, Thermometer, Droplets, Wind, X } from "lucide-react";
 ...
 */
const WeatherBasedSuggestions = memo(({ className = "" }: WeatherBasedSuggestionsProps) => {
  ...
  const { data, isLoading, error, refetch } = useWeatherSuggestions(location, location !== null);
  ...
  useEffect(() => {
    if (useGeolocation && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (_error) => {
          setLocationError("Unable to get your location. Please enter a city name.");
  ...
  const handleRefresh = useCallback(() => {
    if (location) {
      ...
      queryClient.invalidateQueries({ queryKey: ["weather", "suggestions", location] });
      refetch();
  ...
  const handleFavouriteToggle = useCallback(
    (recipe: { id: number; title: string; image: string; imageType: string }, isFavourite: boolean) => {
      if (isFavourite) {
        removeFavourite.mutate(recipe.id, {
          onSuccess: () => {
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-blue-500/70 via-blue-500/50 to-blue-500/30 hover:from-blue-500/80 hover:via-blue-500/60 hover:to-blue-500/40"
                >
                  Search
                </Button>
              </div>
      ...
      {error && (
        <Card className="bg-slate-800/50 border-red-500/30 p-4">
          <CardContent className="p-6 text-center">
            <p className="text-red-400 mb-2">Failed to load weather suggestions</p>
              ...
              <CardContent className="p-6 text-center">
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-300">
                    {data.message || "Daily API limit reached. Recipe suggestions will be available tomorrow."}
                  </p>
                  <p className="text-xs text-gray-400 mt-2">
                    Weather data is still available above. You can still search for recipes manually using the search bar!
                  </p>
                </div>
              </CardContent>
            </Card>
