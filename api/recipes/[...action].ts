 ...
 * - /api/recipes/search → path[0] === "search"
 * - /api/recipes/autocomplete → path[0] === "autocomplete"
 * - /api/recipes/favourite → path[0] === "favourite"
 * 
...
export default async function handler(
  ...
  try {
    ...
    console.error(`❌ [Recipes API] Error (${action}):`, error);
